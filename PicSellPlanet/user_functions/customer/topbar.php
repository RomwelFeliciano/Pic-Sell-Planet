    ...
    <nav class="main-header navbar navbar-expand py-2">
        ...
        <li>
                ...
                ?>
                    <div class="dropdown">
                        <button onclick="showNotif()" class="dropbtn fa-solid fa-bell"><sup style="font-size: 20px;"><?php echo $notif_unread_num ?></sup></button>
                ...
                <?php
                    ...
                    if(!is_null($row['avail_pending']) || !is_null($row['avail_proceed_downpayment']) || !is_null($row['avail_confirmed']) || !is_null($row['avail_completed']) || !is_null($row['avail_cancelled']) || !is_null($row['avail_reschedule_accepted']) || !is_null($row['avail_reschedule_rejected']))
                    {
                        ...
                        if(!is_null($row['avail_reschedule_accepted'])){ $avail_id_notif = $row['avail_reschedule_accepted']; $avail_notif_status = 5; }
                        if(!is_null($row['avail_reschedule_rejected'])){ $avail_id_notif = $row['avail_reschedule_rejected']; $avail_notif_status = 6; }
                        
                        $avail_details =  $conn->query("SELECT a.avail_starting_date_time, a.avail_cancel_reason, s.service_id, s.service_name, u.user_id as sender_id, CONCAT(u.user_first_name , ' ' , u.user_last_name) AS notification_sender FROM tbl_service_avail a LEFT JOIN tbl_service_packages s ON a.service_id = s.service_id RIGHT JOIN `tbl_user_account` u ON s.user_id = u.user_id WHERE a.avail_id = '$avail_id_notif' AND a.user_id = {$_SESSION['login_user_id']}");
                        while($row=$avail_details->fetch_assoc())
                        {
                            $year_month = $ym = date('Y-m', strtotime($row['avail_starting_date_time']));
                            $reason_notif = $row['avail_cancel_reason'];
                        ...
                        if($avail_notif_status == 1)
                        { 
                            $src = "downpayment.png";
                            $stat = "lets you proceed to downpayment";
                            $sentence = $sender_name . ' ' . $stat . ' of your Availed Schedule in ' . $srvc_name_notif;
                        }

                        if($avail_notif_status == 2)
                        { 
                            $src = "confirmed-logo.png";
                            $stat = "confirmed";
                            $sentence = $sender_name. ' ' . $stat . ' your Availed Schedule in ' . $srvc_name_notif;
                        }
    
                        if($avail_notif_status == 3)
                        { 
                        ...
                        { 
                            $src = "cancel-logo.png";
                            $stat = "cancelled";
                            $reason_notif = 'due to reason of "' . $reason_notif . '"';
                            $sentence = $sender_name. ' ' . $stat . ' your Availed Schedule in ' . $srvc_name_notif . ' ' . $reason_notif;
                        }

                        if($avail_notif_status == 5)
                        { 
                            $src = "cancel-logo.png";
                            $stat = "accepted your request of reschedule of ";
                            $sentence = $sender_name. ' ' . $stat . ' your Availed Schedule in ' . $srvc_name_notif;
                        }
                        ...
                        { 
                            $src = "cancel-logo.png";
                            $stat = "rejected your request of reschedule of ";
                            $sentence = $sender_name. ' ' . $stat . ' your Availed Schedule in ' . $srvc_name_notif;
                        }
                ?>
                    <div class="card-notif<?php echo $notif_class ?>" >
                        ...
                        <div>
                            ...
                            <a  class="notif-btn" onclick="location.href='customer_dashboard.php?page=service&l_id=<?php echo $sender_id_notif ?>&srvc_id=<?php echo $service_id_notif ?>&ym=<?php echo $year_month ?>';">Go</a>
                        </div>
                ...
                <?php
                    ...
                    else if(!is_null($row['order_pending']) || !is_null($row['order_confirmed']) || !is_null($row['order_completed']) || !is_null($row['order_cancelled']))
                    {
                        ...
                        $order_details =  $conn->query("SELECT o.order_cancel_reason, o.order_quantity, pr.product_id, pr.product_name, u.user_id as sender_id, CONCAT(u.user_first_name , ' ' , u.user_last_name) AS notification_sender FROM tbl_order o LEFT JOIN tbl_product pr ON pr.product_id = o.product_id RIGHT JOIN `tbl_user_account` u ON pr.user_id = u.user_id WHERE o.order_id = '$order_id_notif' AND o.user_id = {$_SESSION['login_user_id']}");
                        while($row=$order_details->fetch_assoc())
                        {
                            $reason_notif = $row['order_cancel_reason'];
                        ...
                        if($order_notif_status == 1)
                        { 
                            $src = "order-confirmed.png";
                            $stat = "confirmed";
                            $sentence = $sender_name. ' ' . $stat . ' your order of ' . $product_name_notif . ' x' . $order_quant;
                            $link = "&my_orders&confirmed";
                        }
    
                        if($order_notif_status == 2)
                        { 
                            $src = "order-completed.png";
                            $stat = "completed";
                            $sentence = $sender_name. ' ' . $stat . ' your order of ' . $product_name_notif . ' x' . $order_quant;
                            $link = "&history";
                        }
                ...
                ?>
                    <div class="card-notif<?php echo $notif_class ?>" >
                        ...
                        <div>
                            <h6><?php echo $datetime ?></h6>
                            <p><?php echo $sentence ?></p>
                            <a  class="notif-btn" onclick="location.href='customer_dashboard.php?page=market<?php echo $link ?>';">Go</a>
                        </div>
