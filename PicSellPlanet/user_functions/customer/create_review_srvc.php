<?php 
    ...
    <div class="container-fluid">
        <?php
            $s_id = $_GET['srvc_id'];
            $srvc_fdbk = $conn->query("SELECT feedback_message FROM tbl_feedback WHERE user_id = {$_SESSION['login_user_id']} AND service_id = '$s_id' ")->fetch_assoc();
            (isset($srvc_fdbk['feedback_message'])) ? $fdbk_msg = $srvc_fdbk['feedback_message'] : $fdbk_msg = '';
        ?>
        <form action="" id="create_review">
            <input type="hidden" name="service_id" id="service_id" value="<?php echo $s_id ?>">
    ...
    $(document).ready(function () {
        ...
        $('#review_submit').click(function(){
            ...
            {
                $.ajax({
                    url:"ajax.php?action=save_review_service",
                    method:"POST",
                    data: formData,
