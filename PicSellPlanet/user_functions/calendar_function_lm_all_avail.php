<?php
        ...
        if( $row['service_id'] == $_GET['srvc_id'])
        {
            ...
            elseif($row['avail_status'] == 1)
            {
                ...
                else
                {
                    for($i = 0; $i < funcdiff($row['avail_ending_date_time'], $row['avail_starting_date_time']); $i++)
                    {
                        $downpayment[] = createRange($row['avail_starting_date_time'], $row['avail_ending_date_time'])[$i];
                    }
                }
            }
            elseif($row['avail_status'] == 2)
            {
                if($s_nt == $e_nt)
                {
                    $confirmed[] = $s_nt;
                ...
                else
                {
                    for($i = 0; $i < funcdiff($row['avail_ending_date_time'], $row['avail_starting_date_time']); $i++)
                    {
                        $confirmed[] = createRange($row['avail_starting_date_time'], $row['avail_ending_date_time'])[$i];
                    }
                }
            }
            elseif($row['avail_status'] == 5)
            {
                if($s_nt == $e_nt)
                {
                    $resched[] = $s_nt;
    ...
    for ( $day = 1; $day <= $day_count; $day++, $str++) {
        ...
        elseif(in_array($date, $downpayment))
        {
            $week .= '<td  class="downpayment">' . $day;
        }
        elseif(in_array($date, $confirmed))
        {
            $week .= '<td  class="confirmed">' . $day;
        }
        elseif(in_array($date, $resched))
        {
            $week .= '<td  class="resched">' . $day;
        }
